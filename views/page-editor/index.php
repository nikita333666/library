...
<?php
// Регистрируем JavaScript код для работы с редактируемым контентом
$this->registerJs(<<<JS
...
function saveInlineContent(element, content) {
    ...
    fetch('$updateUrl', {
        ...
        body: new URLSearchParams({
            ...
            content: content,
            page: '$currentPage',
            section: 'main' // Добавляем секцию для главной страницы
        })
    })
    ...
    .then(data => {
        if (data.success) {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Ошибка!',
                text: data.message || 'Произошла ошибка при сохранении',
                toast: true,
                position: 'top-end',
                showConfirmButton: false,
                timer: 3000
